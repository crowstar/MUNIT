...
"""
from networks import AdaINGen, MsDis, VAEGen, VGGLoss, GANLoss, FeatLoss
...
class UNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        self.dis_a = MsDis(hyperparameters['input_dim_a'], getIntermFeat=self.ganFeatLoss)  # discriminator for domain a
        self.dis_b = MsDis(hyperparameters['input_dim_b'], getIntermFeat=self.ganFeatLoss)  # discriminator for domain b
        ...
        # initialise feature matching loss
        self.criterionFeat = FeatLoss()
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        # GAN loss
        self.loss_gen_adv_a = self.criterionGAN(self.dis_a(x_ba), True)
        self.loss_gen_adv_b = self.criterionGAN(self.dis_b(x_ab), True)

        # VGG feature matching loss
        self.loss_gen_vgg_a = 0
        self.loss_gen_vgg_b = 0
        if self.vgg_bool == True:
            self.loss_gen_vgg_a = self.criterionVGG(x_a, x_aba)
            self.loss_gen_vgg_b = self.criterionVGG(x_b, x_bab)
            
        # GAN feature matching loss
        self.loss_gen_feat_a = self.criterionFeat(x_a, x_aba) if self.ganFeatLoss else 0
        self.loss_gen_feat_b = self.criterionFeat(x_b, x_bab) if self.ganFeatLoss else 0
    ...
    def dis_update(self, x_a, x_b, hyperparameters):
        ...
        x_ab = self.gen_b.decode(h_a + n_a)
        # D loss
        self.loss_dis_a = self.backward_D_basic(self.dis_a, x_a, x_ba)
        self.loss_dis_b = self.backward_D_basic(self.dis_b, x_b, x_ab)
